...
/** Parses+validates a JSON textarea value against `shape`, falling back to `empty` for a blank field. */
function jsonField<T>(shape: z.ZodType<T>, empty: T) {
    return z.string().transform((raw, ctx) => {
        ...
        if (!result.success) {
            ctx.addIssue({ code: 'custom', message: `Doesn't match the expected shape: ${result.error.issues[0]?.message}` });
            return z.NEVER;
        }
        return result.data;
    });
...
export const BookFormSchema = z.object({
    ...
    title: jsonField(z.array(LineTokenSchema), []),
    subtitle: z.string().trim().min(1, 'A subtitle is required.'),
    teaser: optionalText,
    cover_image_alt: optionalText,
    cover_image_url: optionalText,
    note: optionalText,
    gloss_overrides: jsonField(z.record(z.string(), z.object({ gloss: z.string(), pron: z.string().optional(), source: z.string().optional() })), {}),
...
export const BookPageFormSchema = z.object({
    ...
    lines: jsonField(z.array(BookLineSchema), []),
